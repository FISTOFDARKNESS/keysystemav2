...
exports.handler = async (event) => {
  ...
  if (!r.rows.length) {
    ...
    return {
      statusCode: 404,
      body: "Key not found. Request a new one."
    };
  }
  ...
  if (expired) {
    ...
    await client.end();
    return { statusCode: 410, body: "Key expired. Request a new one." };
  }
    ...
    body: `
      ...
      <head>
        ...
        <style>
          ...
          .key{font-family:Consolas,monospace;font-size:16px;padding:12px;background:#071019;border-radius:8px;margin:12px 0;color:#bff6c9;word-break:break-all}
          small{color:#9fb6c9}
        </style>
      </head>
      <body>
        <div class="box">
          ...
          <small>Expires at: ${new Date(row.expires_at).toLocaleString()}</small>
        </div>
      </body>
      </html>
    `
